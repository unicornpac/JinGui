...
- 根因：公开首页 `/user` 加载统计卡片时自动请求受保护的 `/api/agent/sessions`；该接口的 401 带 `WWW-Authenticate: Basic`，触发浏览器原生密码框。
- 新增 `/api/agent/public-stats`，仅返回条文、病案、训练次数和平均分四项聚合数据，不暴露会话明细。
- 首页改用公开统计接口，同时修复原先 `limit=1` 导致条文和病案总数始终显示为 1 的问题。
- 将认证拆分为两种行为：
  - 管理端页面 `/` 保留 Basic challenge，继续显示浏览器登录框。
  - 管理 API 未认证时只返回 JSON 401，不再意外弹出浏览器密码框。
